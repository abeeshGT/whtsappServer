...
const addDatabaseConnection = async (req, res) => {
   try {
      const { userName, password, dbString, userId } = req.body;
      if (!userName || !password || !dbString || !userId) {
         return res
            .status(400)
      ...
      const response = await updateDatabaseDetails(userId, db);
      if (!response)
         return res
            .status(400)
            .json({ status: "failed", message: "Failed to update db details" });
      res.status(200).json({
         status: "success",
